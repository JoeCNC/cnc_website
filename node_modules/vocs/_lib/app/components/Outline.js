...
export function Outline({ minLevel = 2, maxLevel: maxLevel_ = 3, highlightActive = true, onClickItem, showTitle = true, } = {}) {
    ...
    return (_jsx("aside", { className: styles.root, children: _jsxs("nav", { className: styles.nav, children: [showTitle && _jsx("h2", { className: styles.heading, children: "On this page" }), _jsx(Items, { activeId: highlightActive ? activeId : null, items: items, onClickItem: () => {
                        onClickItem?.();
                        active.current = false;
                        setTimeout(() => {
                            active.current = true;
                        }, 500);
                    }, levelItems: levelItems, setActiveId: setActiveId })] }) }));
}
function Items({ activeId, items, levelItems, onClickItem, setActiveId, }) {
    ...
    return (_jsx("ul", { className: styles.items, children: levelItems.map(({ id, level, text }) => {
            ...
            return (_jsxs(Fragment, { children: [_jsx("li", { className: styles.item, children: _jsx("a", { "data-active": isActive, href: `${pathname}${hash}`, onClick: () => {
                                onClickItem?.();
                                setActiveId(id);
